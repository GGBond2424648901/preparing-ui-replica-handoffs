...
8. 原始画布基线与弹性布局、响应式变体分离。原始画布是测量基线而非固定容器或页面、工作区、模块内容范围的尺寸上限；逐区记录固定/流式/有界流式/内容固有尺寸、最小/基准/最大尺寸、Grid/Flex 伸缩和滚动所有权。宽屏按证据利用可用宽度并分配剩余空间，避免无依据的大面积空白；窄屏和浏览器缩放形成的有效窄视口在达到最小尺寸后依据证据换行、重排或采用整页、模块、混合滚动。响应式只有在有证据或批准时才建立独立变体，禁止根节点整体缩放以及任意压缩或裁切。
