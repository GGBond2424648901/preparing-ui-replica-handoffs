...
原始画布是 1:1 测量基线，不是页面或工作区尺寸上限。内容超出时保持卡片宽度、列比例、文字尺寸、间距和模块位置，可扩展整页、工作区或模块内容范围；根据设计与功能证据采用整页滚动、模块内滚动或两者并存。不得为了塞入截图视口或浏览器而压缩、缩小、重排、移动或裁切。
...
为每个可见区域记录唯一 region ID、`x/y/width/height` 参考边界、父容器、上下左右邻接、对齐、列比例、间距、内边距、最小/最大尺寸、层级和溢出行为。明确溢出由浏览器页面、具体区域或两者共同承担，并记录滚动轴、固定尺寸依据和滚动链关系。坐标是测量基线，不是要求全页绝对定位。

ruleId: PS-006

同时描述适合实现的布局模型（正常流、Grid、Flex、叠层）、锚定关系和滚动所有者。只有设计或功能约束能证明固定视口时，才把滚动限制在模块内部；证据不足时以 `unknown` + `gapId` 保留页面级、区域级和混合策略候选。若像素测量存在误差，记录容差和测量方法，不伪造整数精度。
...
未批准的视口变化默认继承原始几何，但不把原始画布当作最大宽高，也不预设只能整页或只能模块滚动。依据设计与功能证据选择页面级、区域级或混合溢出；证据不足时标为 `candidate`/`proposed` 并关联 `gapId`。任何适配方案单独验收，不能覆盖原生像素基线。
