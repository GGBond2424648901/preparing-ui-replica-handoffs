...
- Treat the original canvas as a native-pixel measurement baseline, not a fixed container or maximum page/workspace size. For every shell, region, grid, and component, record fixed/fluid/bounded/intrinsic sizing, min/base/max dimensions, grow/shrink rules, track ratios, and wide/narrow/zoom behavior. On wider effective viewports, consume available workspace width and distribute surplus space according to evidence instead of leaving accidental dead space; preserve an evidenced max-width when one exists. When space is insufficient, choose reflow, wrapping, page scrolling, region scrolling, or a hybrid from design/function evidence while preserving minimum geometry. Never use whole-page `transform: scale()` or compress, rearrange, or crop merely to fit the captured viewport. 将原始画布视为原生像素测量基线，而不是固定容器或页面/工作区的最大尺寸；逐一记录 Shell、区域、网格和组件的固定/流式/有界流式/内容固有尺寸、最小/基准/最大尺寸、伸缩规则、轨道比例及宽屏/窄屏/缩放行为。有效视口变宽时，应按证据让工作区利用可用宽度并分配剩余空间，避免意外大面积空白；有明确最大宽度证据时保留该上限。空间不足时依据设计与功能证据选择重排、换行、整页滚动、模块内滚动或混合策略，并保持最小几何约束。禁止使用全页 `transform: scale()`，也不得仅为塞入截图视口而压缩、重排或裁切。
