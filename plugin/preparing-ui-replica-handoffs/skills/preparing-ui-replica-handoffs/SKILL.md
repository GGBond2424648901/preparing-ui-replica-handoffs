...
description: "Prepare bilingual, evidence-bounded, desktop-Web-first UI replica handoff packages from design images, including authoritative UI design-language boards. Use when screenshots, Figma exports, generated mockups, component/brand boards, or approved UI images must become precise Chinese-English global, page, micro-visual, and single-application implementation contracts for a downstream Agent."
---
...
4. Extract the shared UI design language before page implementation: principles, materials, tokens, shell, Logo/icon strategy, chart language, motion/state language, micro-visual rules, scope, exceptions, and conflicts. 先提取共享 UI 设计语言，再进入逐页实施：原则、材质、Token、Shell、Logo/图标、图表、动效/状态、微视觉、适用范围、例外与冲突。
...
6. Extract global tokens, shells, components, icons, copy, data classifications, rendering strategies, and reuse decisions. Keep page-local candidates local until evidence supports sharing. 提取全局 token、Shell、组件、图标、文案、数据分类、渲染策略和复用决策；证据不足的候选保持页面局部。
